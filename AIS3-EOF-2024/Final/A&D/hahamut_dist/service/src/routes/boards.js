...
router.get('/board/:id/new-thread', loginRequired, (req, res) => {
  ...
  const categories =
    sql`select id, name from categories where board_id=${req.params.id}`.all()
...
router.get('/board/:id/category', (req, res) => {
  const board = sql(
    `select id, name from boards where id=${req.params.id}`
  ).get()
  if (!board) {
    res.status(404).render('error', {
      title: 'Board not found',
  ...
  const categories = sql(
    `select id, name from categories where board_id=${board.id}`
  ).all()
  res.json(categories)
})
