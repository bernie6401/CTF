...
router.post('/admin/backup', adminRequired, async (req, res) => {
  ...
  for (const tbl of ['boards', 'categories', 'threads', 'posts', 'users']) {
    if (backup[tbl]) {
      await fs.writeFile(
        path.join(tmpdir, tbl + '.json'),
        JSON.stringify(sql(`SELECT * FROM ${tbl}`).all())
      )
    }
  }
  // Patch: filter out `
  if (
    ...
    JSON.stringify(backup).toLowerCase().includes('eval')
  ) {
    res.status(400).render('error', {
      title: 'Bad request',
  ...
  const child = spawn(
    'tar',
    ['-cf', path.resolve(dest), ...Object.keys(backup).map((x) => x + '.json')],
    {
      cwd: tmpdir,
    }
  )
