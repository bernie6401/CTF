...
router.post('/register', (req, res) => {
  ...
  // Patch: Block register as admin
  if (role == 'admin') {
    res.render('register', { title: 'Register', error: 'Invalid role' })
    res.end()
    return
  }
  // End patch
...
router.get('/profile/:id', (req, res) => {
  const page_user =
    sql`select id, username, role, description, profile_picture_url, password from users where id=${req.params.id}`.get()
  if (!page_user) {
    res.render('error', {
      title: 'Error',
  ...
  if (map.get(page_user.password)) {
    console.log(map)
    page_user.password = map.get(page_user.password)
  } else {
    const hash =
      createHash('sha256').update(page_user.password).digest('base64') ?? null
    if (hash) map.set(page_user.password, hash)
    page_user.password = hash
  }
  if (!page_user) {
    res.render('error', {
      title: 'Error',
...
router.post('/profile/:id/password', loginRequired, (req, res) => {
  ...
  if (
    sql`select password from users where id=${req.params.id}`.get().password !==
    old_password
  ) {
