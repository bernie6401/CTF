...
(function (CloudExperienceHost) {
    ...
    (function (AutoPilot) {
        ...
        (function (EnrollmentStatusPage) {
            ...
            function getIsRunningOnHubAsync() {
                return new WinJS.Promise(function (completeDispatch, errorDispatch) {
                    try {
                        CloudExperienceHost.Telemetry.logEvent("Autopilot_EnrollmentStatusPage_getIsRunningOnHubAsync checking isRunningOnHub");

                        let DETECT_RUNNING_ON_HUB_SETTING = "IsRunningOnHub";
                        let autopilotServer = new EnterpriseDeviceManagement.Service.AutoPilot.AutoPilotServer();

                        autopilotServer.getSettingAsync(DETECT_RUNNING_ON_HUB_SETTING).then((result) => {
                            if (result === "True") {
                                completeDispatch(true);
                            } else {
                                completeDispatch(false);
                        ...
                        // If an exception is thrown, return that we are not running on hub
                        CloudExperienceHost.Telemetry.logEvent("Autopilot_GetIsRunningOnHubAsyncFailed", JSON.stringify({ error: err }));
                        completeDispatch(false);
                    }
                });
            }
            if (CloudExperienceHostAPI.FeatureStaging.isOobeFeatureEnabled("AutopilotSurfaceHub22H2")) {
                AutoPilot.EnrollmentStatusPage.getIsRunningOnHubAsync = getIsRunningOnHubAsync;
            }
        ...
        function internalLogEvent(eventName, errorCode, eventMessage, eventMetadata) {
            ...
            CloudExperienceHost.Telemetry.logEvent(eventName, JSON.stringify(eventMetadata));
        }
