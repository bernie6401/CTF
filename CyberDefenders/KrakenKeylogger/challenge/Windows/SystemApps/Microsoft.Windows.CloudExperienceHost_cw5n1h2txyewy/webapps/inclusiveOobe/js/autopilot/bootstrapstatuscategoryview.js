...
define([
    ...
    class bootstrapStatusCategoryView {
                ...
                for (let i = 0; i < this.bootstrapSubcategories.length; i++) {
                    ...
                    if (subcategoryDisposition === this.sessionUtilities.SUBCATEGORY_DISPOSITION_VISIBLE) {
                        ...
                        if (undefined === subcategoryText) {
                            ...
                            subcategoryText = this.sessionUtilities.formatMessage(
        ...
        async updateSubcategoryTextFromActionResultsAsync(subcategory, actionResults) {
            ...
            this.sessionUtilities.logInfoEvent(`BootstrapStatus: Subcategory ID = ${subcategory.getId()}; state = ${actionResults.actionResultState}.`);
        ...
        async startOneActionBatchAsync(previousActionSucceeded, useParallelizableActions, forcedResult, tryingAgain) {
            ...
            this.sessionUtilities.logInfoEvent(`BootstrapStatus: Processing one ${useParallelizableActions ? "parallelizable" : "non-parallelizable"} batch of ${subcategoriesToExecute.length} subcategories...`);
            ...
            for (let i = 0; i < subcategoriesToExecute.length; i++) {
                ...
                let subcategoryText = this.sessionUtilities.formatMessage(
                    this.resourceStrings["BootstrapPageSubcategoryTextTemplate"],
                    currentSubcategory.getTitle(),
                    statusText);
            ...
            for (let i = 0; i < subcategoriesToExecute.length; i++) {
                ...
                let progressCallbackAsync = (progressText) => {
                    if (!this.batchCancelled) {
                        ...
                        if (subcategoryDisposition === this.sessionUtilities.SUBCATEGORY_DISPOSITION_VISIBLE) {
                            let finalMessage = this.sessionUtilities.formatMessage(subcategoryTextTemplate, title, progressText);
                ...
                this.sessionUtilities.logInfoEvent("BootstrapStatus: Starting subcategory {0}...", subcategoriesToExecute[i].getId());
            ...
            for (let i = 0; i < subcategoriesToExecute.length; i++) {
                ...
                if (!this.sessionUtilities.subcategorySucceeded(actionResult.actionResultState)) {
                    if (this.subcategoryNeedsReboot(actionResult.actionResultState)) {
                        ...
                        this.sessionUtilities.logInfoEvent(`BootstrapStatus: Reboot required by subcategory ${currentSubcategory.getId()}.`);
                    } else {
            ...
            if (batchSucceeded && batchRequiresReboot) {
                ...
                this.sessionUtilities.logInfoEvent(`BootstrapStatus: ${useParallelizableActions ? "Parallelizable" : "Non-parallelizable"} batch of ${subcategoriesToExecute.length} subcategories requires a reboot.`);
                ...
                try {
                    ...
                    this.sessionUtilities.logErrorEvent("BootstrapStatus: Navigation to reboot failed.", e);
                ...
                this.sessionUtilities.logInfoEvent(`BootstrapStatus: ${useParallelizableActions ? "Parallelizable" : "Non-parallelizable"} batch of ${subcategoriesToExecute.length} subcategories ${batchSucceeded ? "succeeded" : "failed"}`);
            }
            ...
            if (batchRequiresReboot) {
                ...
                this.sessionUtilities.logErrorEvent("BootstrapStatus: Device failed to reboot within allotted time.", e);
            }
            ...
            if (!batchSucceeded) {
                ...
                this.sessionUtilities.logInfoEvent(`BootstrapStatus: ${this.sessionUtilities.runningInOobe() ? "Device" : "Account"} setup category sent failure results to MDM server.`);
            }
            ...
            if (!previousCategorySucceeded) {
                ...
                return new WinJS.Promise((completeDispatch, errorDispatch, progressDispatch) => {
                    ...
                    return this.setCategoryPropertiesAsync(newCategoryProperties).then(() => {
                        ...
                        this.sessionUtilities.logErrorEvent("BootstrapStatus: startOneActionBatchPromise failed", error);
                    });
                },
                () => {
                    this.sessionUtilities.logInfoEvent("BootstrapStatus: startActionsAsync is cancelled.");
