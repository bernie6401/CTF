...
(() => {
    WinJS.UI.Pages.define("/webapps/inclusiveOobe/view/oobelocalaccount-main.html", {
        init: (element, options) => {
            ...
            return WinJS.Promise.join({ loadCssPromise: loadCssPromise, langAndDirPromise: langAndDirPromise, getLocalizedStringsPromise: getLocalizedStringsPromise, isConnectedToNetworkPromise: isConnectedToNetworkPromise, getPlatformPromise: getPlatformPromise });
        },
        error: (e) => {
        ...
        ready: (element, options) => {
            require(['lib/knockout', 'jsCommon/knockout-helpers', 'legacy/bridge', 'legacy/events', 'oobelocalaccount-vm', 'lib/knockout-winjs'], (ko, KoHelpers, bridge, constants, LocalAccountViewModel) => {
                ...
                let vm = new LocalAccountViewModel(this.resourceStrings, this.isInternetAvailable, this.requirePassword, this.requireRecovery, this.allowOnlineAccount, this.onlineAccountTargetId);
                ko.applyBindings(vm);
