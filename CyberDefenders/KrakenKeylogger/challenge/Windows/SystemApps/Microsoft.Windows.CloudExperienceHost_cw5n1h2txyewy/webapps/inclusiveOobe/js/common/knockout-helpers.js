...
define(['lib/knockout', 'legacy/appObjectFactory',
    ...
    class KnockoutHelpers {
        ...
        registerCustomComponents() {
            ...
            let componentLoader = {
                getConfig: (name, callback) => {
                    if (name.split('-')[0] == 'oobe') {
                        let viewModelConfig = { require: 'jsCommon/' + name + '-vm' };
                        let templateConfig = { require: 'lib/text!viewTemplates/' + name + '-template.html' };
        ...
        static registerDialogComponents() {
            if (dialogComponentsRegistered) { return; }
